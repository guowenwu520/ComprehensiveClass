...
import com.example.vaporwaveVideo.util.MatrixUtils;
...
class TextureRender {
    ...
    public TextureRender(VideoInfo info) {
        ...
        mShow = new NoFilter(getContext().getResources());
        mShow.setMatrix(MatrixUtils.flip(MatrixUtils.getOriginalMatrix(), false, true));
        rotationFilter = new RotationOESFilter(getContext().getResources());
    }
...
//        st.getTransformMatrix(mSTMatrix);//当视频角度不为0时,这里会导致视频方向不对
    ...
    public void surfaceCreated() {
        mProgram = createProgram(OpenGlUtils.uRes(
                "shader/base_record_vertex.sh"),OpenGlUtils.uRes("shader/base_record_fragment.sh"));
        if (mProgram == 0) {
    ...
    private int createProgram(String vertexSource, String fragmentSource) {
        Log.e(TAG, "createProgram: "+vertexSource+"  "+fragmentSource);
